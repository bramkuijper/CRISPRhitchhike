...
// overriding clamp function otherwise stod()
// chokes on output
double Solver::clamp(double const val)
{
    return(std::clamp(val, params.dbl_min_val,params.dbl_max_val));
}
...
void Solver::write_data()
{
            ...
            host_type_idx < 2; ++host_type_idx)
    {
        ...
        // p_S|c and p_S|p
        data_file << clamp(popsize[host_type_idx] / totals_pc[host_type_idx]) << ";";
...
void Solver::write_data_headers()
{
            ...
            host_type_idx < 2; ++host_type_idx)
    {
        ...
        data_file << "dS" << (host_type_idx == P ? "P" : "C") << "dt;";
